...
appears rebuilds the pack and re-sends it to everyone online (`/polymer generate-pack reload`),
then re-sends the equipment of every ovve a player can see once their client reports the pack
loaded (a mixin on the resource-pack response). Rebuilds are batched: a combination the preview
layer can still show waits a minute and a half for company; one it cannot is built within two
seconds.
