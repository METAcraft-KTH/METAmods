...
public class Commands {
	...
	public static final SuggestionProvider<CommandSourceStack> MODERATION_MODE_SUGGESTIONS = (ctx, suggestionsBuilder) -> {
		return SharedSuggestionProvider.suggest(
				ModerationData.getInstance(
						ctx.getSource().getServer()).getValidDefinitionNamesFor(ctx.getSource().getPlayerOrException()
				), suggestionsBuilder
		);
	};

	public static final SimpleCommandExceptionType MODERATION_MODE_INVALID = new SimpleCommandExceptionType(
			Component.literal("That moderation mode does not exist!")
	);
			...
			literal("mod").requires(Permissions.require("metacraft.mod", 3)).then(
				literal("define").requires(Permissions.require("metacraft.mod.define", 3)).then(
					literal("new").then(
						argument("mod", StringArgumentType.string()).executes(ctx -> {
							...
							if (data.getDefinition(name).isPresent()) {
								ctx.getSource().sendFailure(Component.literal("That mode already exists."));
								return 0;
							}
							data.addModeratorDef(new ModeratorModeDefinition(name));
							ctx.getSource().sendSuccess(() -> Component.literal("Created mod definition " + name), true);
							return 1;
						})
					)
				).then(
					literal("remove").then(
						moderationDefinition("mod").executes(ctx -> {
							...
							ctx.getSource().sendSuccess(() -> Component.literal("Removed mod definition " + def.getName()), true);
							return 1;
						})
					)
				).then(
					literal("set-vanish").then(
						moderationDefinition("mod").then(
							argument("vanish", BoolArgumentType.bool()).executes(ctx -> {
								...
								def.setVanish(state);
								ctx.getSource().sendSuccess(() -> Component.literal("Set vanish for " + def.getName() + " to " + state), true);
								return 1;
							})
						)
					)
					...
					literal("set-announce-advancements").then(
						moderationDefinition("mod").then(
							argument("announce-advancements", BoolArgumentType.bool()).executes(ctx -> {
								...
								ctx.getSource().sendSuccess(() -> Component.literal("Set announce-advancements for " + def.getName() + " to " + state), true);
								return 1;
							})
						)
					)
					...
					literal("set-separate-player-data").then(
						moderationDefinition("mod").then(
							argument("separate-player-data", BoolArgumentType.bool()).executes(ctx -> {
								...
								ctx.getSource().sendSuccess(() -> Component.literal("Set separate player data state for " + def.getName() + " to " + state), true);
								return 1;
							})
						)
					)
					...
					literal("set-enter-command").then(
						moderationDefinition("mod").then(
							CommandUtil.command("arg", 10, dispatcher, (ctx, command) -> {
								...
								net.minecraft.commands.Commands.validateParseResults(ctx.getSource().getServer().getCommands().getDispatcher().parse(command, ctx.getSource()));
								mod.setEnterCommand(command);
								ctx.getSource().sendSuccess(() -> Component.literal("Set enter command to " + command + " for " + mod.getName()), true);
								return 1;
							})
						)
					)
					...
					literal("set-exit-command").then(
						moderationDefinition("mod").then(
							CommandUtil.command("arg", 10, dispatcher, (ctx, command) -> {
								...
								ctx.getSource().sendSuccess(() -> Component.literal("Set exit command to " + command + " for " + mod.getName()), true);
								return 1;
							})
						)
					)
					...
					literal("clear-enter-command").then(
						moderationDefinition("mod").executes(ctx -> {
							...
							ctx.getSource().sendSuccess(() -> Component.literal("Removed enter command from " + mod.getName()), true);
							return 1;
						})
					)
				).then(
					literal("clear-exit-command").then(
						moderationDefinition("mod").executes(ctx -> {
							...
							ctx.getSource().sendSuccess(() -> Component.literal("Removed exit command from " + mod.getName()), true);
							return 1;
						})
					)
				).then(
					literal("get").then(
						moderationDefinition("mod").executes(ctx -> {
							var def = getModerationDefinition(ctx, "mod");
							ctx.getSource().sendSuccess(def::toText, false);
							return 1;
						})
					)
				)
				...
				literal("as").then(
					moderationDefinition("mod").executes(ctx -> {
						...
						ctx.getSource().sendSuccess(() -> Component.literal("You are now in " + def.getName() + " mode."), true);
						return 1;
					})
				)
			).then(
				literal("set-default").then(
					moderationDefinition("mod").requires(CommandSourceStack::isPlayer).executes(ctx -> {
						var player = ctx.getSource().getPlayerOrException();
						var def = getModerationDefinition(ctx, "mod");
						((ModerationPlayerData) player).METAcraft_Moderation$setDefaultModerationMode(def.getName());
						ctx.getSource().sendSuccess(() -> Component.literal("Set default moderator mode to " + def.getName()), false);
						return 1;
					})
				).executes(ctx -> {
					var player = ctx.getSource().getPlayerOrException();
					((ModerationPlayerData) player).METAcraft_Moderation$setDefaultModerationMode(null);
					ctx.getSource().sendSuccess(() -> Component.literal("Removed default moderator mode."), false);
					return 1;
				})
			).then(
				literal("exit").executes(ctx -> {
					...
					ctx.getSource().sendSuccess(() -> Component.literal("You are no longer in moderator mode."), true);
					return 1;
				})
			).executes(ctx -> {
				var player = ctx.getSource().getPlayerOrException();
				...
				if (defName.isEmpty()) {
					ctx.getSource().sendSuccess(() -> Component.literal("You have no default mode set!"), false);
					return 0;
				}
				if (PlayerModerationState.setModeratorMode(player, defName.get())) {
					ctx.getSource().sendSuccess(() -> Component.literal("You are now in " + defName.get() + " mode."), true);
					return 1;
				} else {
	...
	static ArgumentBuilder<CommandSourceStack, ?> moderationDefinition(String arg) {
		return argument(arg, StringArgumentType.string()).suggests(MODERATION_MODE_SUGGESTIONS);
	}

	static ModeratorModeDefinition getModerationDefinition(CommandContext<CommandSourceStack> ctx, String arg) throws CommandSyntaxException {
		var name = StringArgumentType.getString(ctx, arg);
		if (!PlayerModerationState.canEnterModerationMode(ctx.getSource().getPlayerOrException(), name)) {
			throw MODERATION_MODE_INVALID.create();
		}
